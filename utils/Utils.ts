...
import { Language } from "../languages/Translator";
...
export class Utils {
     ...
     */
    public static getClientLanguage(request: Request): Language {
        let extractedLanguage: Language = Language.SPANISH;

        if ((request.headers['accept-language'] as Language) == Language.SPANISH || (request.headers['accept-language'] as Language) == Language.ENGLISH)
            extractedLanguage = (request.headers['accept-language'] as Language);
     ...
     */
    public static getNowInUtc(): Date {
        const nowDate: Date = new Date();
        const nowInUtc: number = Date.UTC(nowDate.getUTCFullYear(), nowDate.getUTCMonth(),
    ...
    */
     ...
     * 
     */
    public static convertToUTC(dateString) {
     ...
     * 
     */
    public static addDay(dateString) {
        const dateParts = dateString.split('/');
        const year = dateParts[0];
        const month = dateParts[1] - 1;
    ...
    public static isFutureDate(dateString) {
    ...
    // Funcion para validar si la fecha es futura
    public static areDistinctNumbers(num1, num2, num3) {
    ...
    public static coincidenDosNumeros(arr1, arr2) {
    ...
    public static getHour(dateString) {
        const date = new Date(dateString);
        return date.getUTCHours();
    }
    public static  joinDateAndHour(fecha, hora) {
